...
# Define the function for step 4
function_for_step_4 <- function(data) {
    ...
    list(noncounty = data_noncounty2, county = data_county2)    
}

# Process the data
data_noncounty2 <- data2$noncounty
data_county2 <- data2$county

# Define the function for step 5
...
# Define the function for step 6
function_for_step_6 <- function(data_noncounty2) {
  ...
  # Convert area_name to upper case and classify the states
  data_noncounty2 <- data_noncounty2 |>
    mutate(area_name = toupper(area_name)) |>
    mutate(division = ifelse(area_name == "UNITED STATES", "ERROR", 
                             ifelse(area_name %in% Northeast, "Northeast", 
                                    ifelse(area_name %in% Midwest, "Midwest", 
                                           ifelse(area_name %in% South, "South",
                                                  ifelse(area_name %in% West, "West", "CHECK"))))))
  
  return(data_noncounty2)
}

# Process the data
data_noncounty2 <- function_for_step_5(data_noncounty2)
```
